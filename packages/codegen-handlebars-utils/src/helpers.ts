...
export const initHandlebarsCodegenHelpers = (primitivesMap: { [primitiveName: string]: string }, schemaContext: SchemaTemplateContext, fileExtensions: string = null) => {
  registerHelper('toPrimitive', function (type) {
    return primitivesMap[type] || type || '';
  });
  ...
  registerHelper('eachImport', function (context: any, options: { fn: Function }) {
    ...
    if (context.variables) {
      context.variables.forEach((variable: Variable) => {
        if (!primitivesMap[variable.type]) {
    ...
    for (let i = 0, j = imports.length; i < j; i++) {
      ret = ret + options.fn(imports[i], {
        data: {
          withExtension: imports[i] + (fileExtensions ? ('.' + fileExtensions) : ''),
        },
      });
    }
